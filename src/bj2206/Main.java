...
//        System.out.println(count[ySize-1][xSize-1][0]);
    ...
    public static void bfs(int y, int x){
        ...
        while(queue.size()!=0){
            ...
            if(!(next.y+1>=ySize)){
                if(graph[y+1][x]==1&&state==0){
                    queue.add(new Point(x,y+1,1));
                    bfsIsVisited[y+1][x][1]=true;
                    count[y+1][x][1]=count[y][x][0]+1;
                }
                if(graph[y+1][x]==0) {
                    if(state==0&&!bfsIsVisited[y+1][x][0]){
                        queue.add(new Point(x, y + 1,0));
                        bfsIsVisited[y + 1][x][0] = true;
                        count[y + 1][x][0] = count[y][x][0] + 1;
                    } else if(state==1&&!bfsIsVisited[y+1][x][1]){
                        queue.add(new Point(x, y + 1,1));
                        bfsIsVisited[y + 1][x][1] = true;
                        count[y + 1][x][1] = count[y][x][1] + 1;
                    }
                }
            }
            if(!(next.x+1>=xSize)){
                if(graph[y][x+1]==1&&state==0){
                    queue.add(new Point(x+1,y,1));
                    bfsIsVisited[y][x+1][1]=true;
                    count[y][x+1][1]=count[y][x][0]+1;
                }
                if(graph[y][x+1]==0) {
                    if(state==0&&!bfsIsVisited[y][x+1][0]){
                        queue.add(new Point(x+ 1, y ,0));
                        bfsIsVisited[y][x+ 1][0] = true;
                        count[y][x+ 1][0] = count[y][x][0] + 1;
                    } else if(state==1&&!bfsIsVisited[y][x+1][1]){
                        queue.add(new Point(x + 1, y,1));
                        bfsIsVisited[y][x + 1][1] = true;
                        count[y][x + 1][1] = count[y][x][1] + 1;
                    }
                }
            }
            if(!(next.y-1<0)){
